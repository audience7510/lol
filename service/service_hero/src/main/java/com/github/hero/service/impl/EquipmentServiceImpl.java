...
@Service
public class EquipmentServiceImpl extends ServiceImpl<EquipmentMapper, Subject> implements IEquipmentService {
    @Override
    ...
    public List<OneSubject> getAllOneTwoSubject() {
        ...
        for (int i = 0; i < oneSubjectList.size(); i++) { //遍历oneSubjectList集合
            ...
            Subject equipmentSubject = oneSubjectList.get(i);
            ...
            for (int m = 0; m < twoSubjectList.size(); m++) {
                //获取每个二级分类
                Subject subject = twoSubjectList.get(m);
